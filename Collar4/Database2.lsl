...
default
{
    ...
    // handle the response from the crime database
    {
        ...
        if(listRequestIndex == -1 || status != 200) // does not process the response if this script did not send a request or the Web API does not work correctly
        {
            if(status != 200 && listRequestIndex != -1)
            {
